...
for d in domains:
    try:
        url = f"https://api.vworld.kr/req/data?key=' + os.environ.get('VWORLD_API_KEY', '') + '&domain={d}&service=data&request=GetFeature&data=LT_C_BULD_INFO&geomFilter=BBOX(127.0345,37.501,127.0355,37.502)&geometry=true&crs=EPSG:4326&format=json&size=10"
